...
export default function PostDetail({post}) {
  ...
  const getObj = (index, objArr) => {
    ...
    switch (objArr.type) {
      ...
      case 'heading-four':
        return <h1 key={index} className="text-2xl font-semibold mb-4 mt-10">{objArr.children.map((obj, i) => getType(obj, i))}</h1>;
      case 'heading-five':
        return <h1 key={index} className="text-xl font-semibold mb-4 mt-10">{objArr.children.map((obj, i) => getType(obj, i))}</h1>;
      case 'heading-six':
        return <h1 key={index} className="text-lg font-semibold mb-4 mt-10">{objArr.children.map((obj, i) => getType(obj, i))}</h1>;
